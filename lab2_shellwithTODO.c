...
int process_redirect(int argc, char** argv, int *fd) {
    ...
    while(i < argc) {
        ...
        if(strcmp(argv[i], ">") == 0) {
            ...
            //TODO: 打开输出文件追加写入
            tfd = open(argv[i+1],O_APPEND|O_CREAT,0666);
            if(tfd < 0) {
                printf("open '%s' error: %s\n", argv[i+1], strerror(errno));
            } else {
                //TODO:输出重定向
...
int main() {
    ...
    while (1) {
        ...
        for(count_semicolon=0;count_semicolon<256,count_semicolon++)
        {
            if(cmdline[count_semicolon]==';') cmdline[count_semicolon]='\n';
        }
        //todo结束
        strtok(cmdline, "\n");//将cmdline中\n之间的内容提取
        ...
        /* 拆解命令行 */
        cmd_count = split_string(cmdline, "|", commands);//被分割的字符串为cmdline，分割依据为|，得到的结果为commands，包含了cmd_count条指令
        ...
        if(cmd_count == 0) {
            ...
            int fd[2];
            /* TODO:处理参数，分出命令名和参数
             * 需要处理的参数：argc、argv、fd
             * argc为命令的参数数目
             * argv为命令对应的一个包含名称和参数的数组，argv[0]为命令名，后面为参数
             * fd为输入输出的端口，其中fd[0]为读端口，fd[1]为写端口
             */
